...
async def test_select_option_masks_stale_readback_until_second_update(
    ...
    assert entity.current_option == CHARGE_PRIORITY_LOWEST_COST
    await entity.async_select_option(CHARGE_PRIORITY_COMPLETE_CHARGE)
    assert entity.current_option == CHARGE_PRIORITY_COMPLETE_CHARGE  # optimistic

    # The write's own racy refresh - read-back still shows the pre-write value.
    entity._handle_coordinator_update()
    assert entity.current_option == CHARGE_PRIORITY_COMPLETE_CHARGE  # still masked

    # The natural next poll actually reflects the write - now trusted.
    coordinator.data = {PPID: make_charger(max_price=0.30, tariff_windows=_windows())}
    entity._handle_coordinator_update()
    assert entity.current_option == CHARGE_PRIORITY_COMPLETE_CHARGE


async def test_select_option_self_corrects_when_backend_contradicts(hass: HomeAssistant) -> None:
    coordinator = make_coordinator(
        hass, {PPID: make_charger(max_price=0.10, tariff_windows=_windows())}
    )
    coordinator.async_request_refresh = AsyncMock()
    ...
    coordinator.data = {PPID: make_charger(max_price=0.10, tariff_windows=_windows())}
    entity._handle_coordinator_update()
    assert entity.current_option == CHARGE_PRIORITY_LOWEST_COST
