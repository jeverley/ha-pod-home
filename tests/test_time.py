...
async def test_ready_by_set_value_forces_vehicle_fetch(hass: HomeAssistant) -> None:
    vehicle = make_vehicle(id="v1", intent_charge_kwh=42.0)
    coordinator = make_coordinator(hass, {PPID: make_charger(vehicle=vehicle)})
    coordinator.async_request_refresh = AsyncMock()
...
async def test_ready_by_masks_stale_readback_until_second_update(hass: HomeAssistant) -> None:
    vehicle = make_vehicle(id="v1", intent_charge_by_time="07:00:00", intent_charge_kwh=42.0)
    coordinator = make_coordinator(hass, {PPID: make_charger(vehicle=vehicle)})
    coordinator.async_request_refresh = AsyncMock()
    entity = time_platform.PodHomeVehicleReadyByTime(coordinator, "v1")
    ...
    await entity.async_set_value(datetime.time(8, 0, 0))
    assert entity.native_value == datetime.time(8, 0, 0)  # optimistic

    entity._handle_coordinator_update()  # the write's own racy refresh, still stale
    assert entity.native_value == datetime.time(8, 0, 0)  # still masked

    coordinator.data = {
        PPID: make_charger(
            vehicle=make_vehicle(id="v1", intent_charge_by_time="08:00:00", intent_charge_kwh=42.0)
        )
    }
    entity._handle_coordinator_update()
    assert entity.native_value == datetime.time(8, 0, 0)
