...
def test_current_charging_seconds_sums_only_charging_windows():
    """Smart Charging path: schedule_events built via smart_schedule_events(), which already
    filters to CHARGING-type windows - current_charging_seconds() itself is mode-agnostic."""
...
def test_current_charging_seconds_clips_to_session_and_now():
...
def test_current_charging_seconds_none_when_no_schedule_no_override():
    """schedule_events=None (not []) signals no schedule was available at all - the case that
    must return None, distinct from a real, empty events list (see the real-zero test above)."""
    now = datetime.datetime(2026, 1, 1, tzinfo=UTC)
...
def test_current_charging_seconds_zero_is_a_real_answer_not_none():
    """No CHARGING window has started yet within the session - a real 0, distinct from the
    None returned when there's no schedule/override at all."""
    session_start = datetime.datetime(2026, 1, 1, 0, 0, tzinfo=UTC)
...
def test_current_charging_seconds_override_active_from_session_start():
    session_start = datetime.datetime(2026, 1, 1, 0, 0, tzinfo=UTC)
...
def test_current_charging_seconds_override_midsession_with_schedule_before_it():
    """Schedule contributes before the override starts; the two don't overlap here, so the
    total is a plain sum - see the overlap-merge test below for when they DO overlap."""
    session_start = datetime.datetime(2026, 1, 1, 0, 0, tzinfo=UTC)
    override_start = datetime.datetime(2026, 1, 1, 1, 0, tzinfo=UTC)
    now = datetime.datetime(2026, 1, 1, 2, 0, tzinfo=UTC)
    schedule_events = [
        (
...
def test_current_charging_seconds_overlap_between_schedule_and_override_not_double_counted():
    session_start = datetime.datetime(2026, 1, 1, 0, 0, tzinfo=UTC)
    override_start = datetime.datetime(2026, 1, 1, 0, 15, tzinfo=UTC)
...
def test_current_charging_seconds_override_start_before_session_start_is_clipped():
    session_start = datetime.datetime(2026, 1, 1, 0, 0, tzinfo=UTC)
    now = datetime.datetime(2026, 1, 1, 1, 0, tzinfo=UTC)
