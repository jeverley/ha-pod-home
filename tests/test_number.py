...
async def test_target_charge_set_value_forces_vehicle_fetch(hass: HomeAssistant) -> None:
    ...
    coordinator = make_coordinator(hass, {PPID: make_charger(vehicle=vehicle)})
    coordinator.async_request_refresh = AsyncMock()
    coordinator.request_vehicles_fetch = MagicMock()
    entity = number.PodHomeVehicleTargetChargeNumber(coordinator, "v1")

    await entity.async_set_native_value(72.6)

    coordinator.request_vehicles_fetch.assert_called_once()


async def test_target_charge_masks_stale_readback_until_second_update(
    hass: HomeAssistant,
) -> None:
    vehicle = make_vehicle(id="v1", charge_limit_percent=80)
    coordinator = make_coordinator(hass, {PPID: make_charger(vehicle=vehicle)})
    coordinator.async_request_refresh = AsyncMock()
    entity = number.PodHomeVehicleTargetChargeNumber(coordinator, "v1")
    ...
    await entity.async_set_native_value(72.6)
    assert entity.native_value == 73  # optimistic, before any coordinator update at all

    # Simulate the write's own racy refresh landing with the read-back still stale.
    entity._handle_coordinator_update()
    assert entity.native_value == 73  # still masked

    # The natural next poll, whether it confirms or contradicts, is trusted.
    coordinator.data = {PPID: make_charger(vehicle=make_vehicle(id="v1", charge_limit_percent=73))}
    entity._handle_coordinator_update()
    assert entity.native_value == 73


async def test_target_charge_self_corrects_when_backend_contradicts(
    hass: HomeAssistant,
) -> None:
    vehicle = make_vehicle(id="v1", charge_limit_percent=80)
    coordinator = make_coordinator(hass, {PPID: make_charger(vehicle=vehicle)})
    ...
    await entity.async_set_native_value(72.6)
    entity._handle_coordinator_update()
    # Something else changed it in the meantime - the second update is trusted regardless.
    coordinator.data = {PPID: make_charger(vehicle=make_vehicle(id="v1", charge_limit_percent=50))}
    entity._handle_coordinator_update()
